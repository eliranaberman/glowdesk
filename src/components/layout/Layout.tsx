...
const Layout = ({ children }: LayoutProps) => {
  ...
  // For debugging
  console.log("Layout rendering with user:", user?.id, "Connection error:", connectionError);

  const getPageTitle = (): string => {
    const path = location.pathname;
    
    if (path === '/' || path === '/dashboard') return 'דשבורד';
    ...
    if (path === '/social-media-meta') return 'חיבור Meta API';
    ...
    if (path === '/marketing') return 'שיווק';
    if (path === '/portfolio') return 'גלריה';
    if (path === '/users') return 'ניהול משתמשים';
    if (path === '/user-roles') return 'תפקידים והרשאות';
    if (path === '/user-profile') return 'פרופיל משתמש';
