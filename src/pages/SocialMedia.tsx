...
import { Plus, Loader2, LayoutDashboard, MessageSquare, Send, BarChart3, BrainCircuit, Settings, Target } from "lucide-react";
  ...
  const getButtonText = () => {
    switch (activeTab) {
      ...
      case "posts":
        return "נהל קמפיינים";
      case "inbox":
        return "צור תוכן עם AI";
      case "dashboard":
        return "רענן חיבורים";
      default:
        return null;
      // No button for connections
    }
  };
  const buttonText = getButtonText();
  return <div className="space-y-4 sm:space-y-6 p-3 sm:p-6 max-w-7xl mx-auto" dir="rtl">
      <div className="flex flex-col sm:flex-row sm:justify-center sm:items-center gap-4">
        <h1 className="text-xl sm:text-2xl font-semibold text-center tracking-tight">מדיה חברתית ושיווק</h1>
        
        {buttonText && <div className="flex flex-col sm:flex-row gap-2 sm:gap-2 justify-center items-center">
      ...
      <Tabs defaultValue={activeTab} value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className={`grid gap-1 w-full mb-4 mx-auto ${isMobile ? 'grid-cols-3 h-auto' : 'grid-cols-7 h-12'}`}>
          <TabsTrigger value="connections" className={`text-xs sm:text-sm py-2 sm:py-2.5 font-medium flex gap-1 sm:gap-1.5 justify-center items-center ${isMobile ? 'min-h-[44px] flex-col px-2' : 'min-h-[44px]'}`}>
            <Settings className="h-4 w-4" />
            <span className={isMobile ? 'text-xs' : ''}>חיבורים</span>
          </TabsTrigger>
          
          <TabsTrigger value="ai-tools" className={`text-xs sm:text-sm py-2 sm:py-2.5 font-medium flex gap-1 sm:gap-1.5 justify-center items-center relative ${isMobile ? 'min-h-[44px] flex-col px-2' : 'min-h-[44px]'}`}>
          ...
          {!isMobile && <>
              <TabsTrigger value="campaigns" className="text-sm py-2.5 font-medium flex gap-1.5 justify-center items-center min-h-[44px]">
                <Target className="h-4 w-4" />
                <span>קמפיינים</span>
              </TabsTrigger>
              
              <TabsTrigger value="analytics" className="text-sm py-2.5 font-medium flex gap-1.5 justify-center items-center min-h-[44px]">
                <BarChart3 className="h-4 w-4" />
                <span>אנליטיקס</span>
              </TabsTrigger>
              
              <TabsTrigger value="posts" className="text-sm py-2.5 font-medium flex gap-1.5 justify-center items-center min-h-[44px]">
                <Send className="h-4 w-4" />
                <span>פרסום</span>
              </TabsTrigger>
              
              <TabsTrigger value="inbox" className="text-sm py-2.5 font-medium flex gap-1.5 justify-center items-center min-h-[44px]">
                <MessageSquare className="h-4 w-4" />
                <span>הודעות</span>
              </TabsTrigger>
            </>}
          
          <TabsTrigger value="dashboard" className={`text-xs sm:text-sm py-2 sm:py-2.5 font-medium flex gap-1 sm:gap-1.5 justify-center items-center ${isMobile ? 'min-h-[44px] flex-col px-2' : 'min-h-[44px]'}`}>
            <LayoutDashboard className="h-4 w-4" />
            <span className={isMobile ? 'text-xs' : ''}>דשבורד</span>
          </TabsTrigger>
        </TabsList>

        {isMobile && <div className="flex gap-2 mb-4 overflow-x-auto pb-2 justify-start scrollbar-hide">
            {[{
          key: "campaigns",
          icon: Target,
          label: "קמפיינים"
          ...
          icon: Icon,
          label
        }) => <Button key={key} variant={activeTab === key ? "default" : "outline"} size="sm" onClick={() => setActiveTab(key)} className="flex items-center justify-center gap-1.5 whitespace-nowrap min-h-[44px] px-4 flex-shrink-0">
        ...
        <TabsContent value="analytics">
          <AnalyticsContent analyticsData={analyticsData} marketingStats={marketingStats} isLoading={isLoading} />
        </TabsContent>
        ...
        <TabsContent value="inbox">
          <UnifiedInbox messages={messages} onMarkAsRead={handleMarkAsRead} onReply={handleReply} />
        </TabsContent>

        <TabsContent value="dashboard">
          <DashboardContent connectedAccounts={connectedAccounts} connectPlatform={connectPlatform} handleOpenInbox={handleOpenInbox} messages={messages} marketingStats={marketingStats} isLoading={isLoading} />
        </TabsContent>
      </Tabs>
    </div>;
};
