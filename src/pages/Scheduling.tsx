...
const Scheduling = () => {
  ...
  const generateRandomAppointments = () => {
    ...
    const clientNames = ['שרה כהן', 'אמילי לוי', 'ליאת ונג', 'מיכל אברהם', 'רחל גולן', 'דנה ישראלי', 'יעל מור', 'נופר דהן', 'טלי ברק', 'מירי אלון', 'רוני שטרן', 'נועה אדלר', 'קרן לוי', 'דפנה גבאי', 'הילה שגיא', 'מאיה ברגר', 'שירה אוחנה', 'איילת בן דוד', 'רותם זהבי', 'עדי מזרחי', 'שני גרינברג', 'ליטל כץ', 'אורטל נחום', 'גלי אשכנזי', 'יפעת אוזן', 'לירון פישר', 'ענבר חן', 'חני לויד', 'הודיה פרץ', 'עדן שפירא', 'אביגיל מנחם', 'ספיר דנינו', 'אופיר יעקב', 'חן אסולין', 'ליאור גולדשטיין', 'רבקה כהן', 'סיגל לוי', 'אורית שלום', 'נטע ברזילי', 'גלית גוטמן', 'שלומית מלכה', 'מיטל יצחקי', 'תמי ממן', 'ורד אזולאי', 'טובה שמעוני'];
    const services = [{
      name: 'מניקור ג\'ל',
      duration: 60,
      color: '#F2FCE2',
    ...
    for (let hour = 8; hour <= 19; hour++) {
      ...
      if (hour < 19) {
        // Don't add 19:30 as it would go beyond business hours
  ...
  useEffect(() => {
    const filterAppointmentsByDate = () => {
      ...
      const filtered = allAppointments.filter(appointment => {
        ...
        return appointment.date.getFullYear() === selectedDate.getFullYear() && appointment.date.getMonth() === selectedDate.getMonth() && appointment.date.getDate() === selectedDate.getDate();
      });
  ...
  return <div dir="rtl">
      ...
      <div className={`space-y-${isMobile ? '4' : '6'}`}>        
        <GanttChart appointments={allAppointments} date={selectedDate} onDateChange={setSelectedDate} />
      </div>
    </div>;
};
